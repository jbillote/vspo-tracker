...
class YouTubeService {
  ...
  public async getVideos(streamerName: string, date: DateTime): Promise<YouTubeVideo[]> {
    ...
    const videos: YouTubeVideo[] = await this.getVideosFromCache(streamerId, date)
    ...
    const client: YouTube = new YouTube(env.GOOGLE_API_KEY!)

    const videosResp: Record<string, any>[] = await client.getVideos(streamerId, date)

    videosResp.forEach((video: Record<string, any>) => {
      videos.push({
        id: video.id.videoId,
        member: streamerId,
  ...
  private async getStreamerId(streamerName: string): Promise<string> {
    ...
    return streamers.streamers
      .find((member: any) => member.name.toLowerCase() === streamerName)
      .youtube.toString()
  }
